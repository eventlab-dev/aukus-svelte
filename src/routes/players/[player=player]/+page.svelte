<script lang="ts">
	...
	const { playersByUrlHandle } = playersStore;
	const { moves } = playersMovesStore;

	const playerName = $page.params.player;

	const player = $derived(playersByUrlHandle[playerName!]);
	const playerMoves = $derived(moves.filter((move) => move.player_id === player.id));
	...
	<div class="mx-auto flex w-fit flex-col items-center" in:fade>
		...
		<div class="mb-[30px] flex flex-col items-center gap-5">
			...
			<Summary
				...
				gameImage={player.current_game_image || ''}
